      ...
      if(start_kos){ //start kos
        ...
        if(room_id){
          result.start_kos = await sq.query(`
            ...
            from history h
              inner join room r on r.id = :id and r.id = h.room_id 
            ...
            where 
              ...
              (start_kos < timestamp :start_kos and start_kos + interval '1 month' * p.duration > timestamp :start_kos + interval '1 month' * p.duration)
            limit 1
          `, {
            replacements: {start_kos, id: room_id}, 
