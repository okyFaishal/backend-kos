...
class Controller {
  static async register(req, res, next) {
    try {
      ...
      if(!(req.body.password && req.files.image_ktp && req.body.birth_date && username && email && contact && nik && birth_place && religion && gender && status)) next({status: 400, message: 'Lengkapi Data'}) 
      ...
      if(/\D/.test(contact)) next({status: 400, message: 'contact tidak valid'})
      else if(/\D/.test(emergency_contact)) next({status: 400, message: 'emergency contact tidak valid'})
      else if(/\D/.test(nik)) next({status: 400, message: 'nik tidak valid'})
      else if(/\D/.test(generation)) next({status: 400, message: 'generation tidak valid'})
      else if(!/\w+@\w+\.\w+/.test(email)) next({status: 400, message: 'email tidak valid'})
      else if(!/mahasiswa|kerja/.test(status)) next({status: 400, message: 'status tidak valid'})
      else if(birth_date == 'Invalid Date') next({status: 400, message: 'birth date tidak valid'})
      ...
      next({status: 200, message: 'success create acount', data: {...result.dataValues, token}})
    } catch (error) {
  ...
  static async login(req, res, next) {
    try {
      ...
      if(!(password && email)) next({status: 400, message: 'Lengkapi Data'})
      let dataCek = await user.findOne({where:{email}}) //ambil data sesuai email

      if(!dataCek) next({status: 402, message: 'Email Belum Terdaftar'})
      else if(!compare(password, dataCek.password)) next({status: 400, message: 'Password Tidak Cocok'})
      ...
      next({status: 200, message: 'success login', data: {...dataCek, token}})
    } catch (error) {
      next({status: 500, data: error})
    }
  }
  static async updateProfile(req, res, next) {
    try {
      ...
      if(contact && /\D/.test(contact)) next({status: 400, message: 'contact tidak valid'}) 
      if(emergency_contact && /\D/.test(emergency_contact)) next({status: 400, message: 'emergency contact tidak valid'}) 
      if(nik && /\D/.test(nik)) next({status: 400, message: 'nik tidak valid'}) 
      if(generation && /\D/.test(generation)) next({status: 400, message: 'generation tidak valid'}) 
      if(email && !/\w+@\w+\.\w+/.test(email)) next({status: 400, message: 'email tidak valid'}) 
      if(status && !/mahasiswa|kerja/.test(status)) next({status: 400, message: 'status tidak valid'}) 
      if(birth_date && birth_date == 'Invalid Date') next({status: 400, message: 'birth date tidak valid'}) 
  ...
  //       else next({status: 400, message: 'tidak bisa menghapus akun admin'}) 
  //     }

  //     let result = await user.destroy({where: {id}})
  //     if(result == 0) next({status: 400, message: 'user tidak ditemukan'}) 
